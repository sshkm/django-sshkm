...
from keymaster.forms import UserForm
...
def SettingsList(request):
    ...
    context = {'users': users}
    return render(request, 'keymaster/settings/list.html', context)
...
#        userform = UserForm()
#
#    return render(request, 'keymaster/settings/passwordreset.html', {
#        'userform': userform,
#    })
...
def CreateUser(request):
    if request.POST.get('password') != request.POST.get('confirm'):
        ...
        if request.POST.get('is_staff'):
            user.is_staff = 1
        user.save()
        #return render(request, 'keymaster/settings/list.html')
        return HttpResponseRedirect(reverse('SettingsList'))
